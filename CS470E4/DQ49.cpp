// Question 49. Leetcode 1584. Min Cost to Connect All Points, Difficulty: Medium
// You are given an array points representing integer coordinates of some points on a 2D-plane, where points[i] = [xi, yi].
// The cost of connecting two points [xi, yi] and [xj, yj] is the manhattan manHattendistance between them: |xi - xj| + |yi - yj|, where |val| denotes the absolute value of val.
...
void dfsOrder(int start, vector<bool>& visited, vector<vector<double>> finalMat) //o(V+E)
{
...
// |xi - xj| + |yi - yj|
double manHattendistance(int x1, int y1, int x2, int y2){ //O(1)
...
void sumDistance(int arr[], int n, vector<vector<double>> &given, double &a) //O(V)
{

	for (int i = 1; i < n; ++i){
         //printf("\n%d - %d\n", arr[i], i);
        //cout<<"dist:"<<manHattendistance(given[arr[i]][1], given[arr[i]][2], given[i][1], given[i][2] ) <<endl;
       
        
        a +=manHattendistance(given[arr[i]][1], given[arr[i]][2], given[i][1], given[i][2] ); //++
    }
...
void PrimMST(struct Graph* graph, vector<vector<double>>& finalMat, double& sum, vector<vector<double>>given)
{
	...
	//printArr(parent, V);
    sumDistance(parent, V, given, sum); //++
    //void sumDistance(int arr[], int n, vector<vector<int>> given, int &a) //O(V)
...
int main()
{
    ...
    //weight = manHattendistance bewteen source and dest
    //every node is connect to every other
    //Given V={a: (1,3), b:(1,1),c: (0,0), d: (3,3), e: (4,2), f: (3,1), g: (5,1)}, h{2,-1}
   //x = givenPoints[a,b,c][x][y]
    //vector<vector<double>> givenPoints{{0,0,0},{1,2,2},{2,3,10},{3,5,2},{4,7,0}};

     vector<vector<double>> givenPoints{{0,0,0},{1,2,2},{2,3,10},{3,5,2},{4,7,0}};
    int V = givenPoints.size();
    ...
    for(int i=0;i<V;i++){
        for(int j=0;j<V;j++){
               // cout<<"givenPoints:("<<i<<")("<<j<<"):"<<manHattendistance(givenPoints[i][1],givenPoints[i][2], givenPoints[j][1], givenPoints[j][2])<<endl;
                addEdge(graph, i, j, manHattendistance(givenPoints[i][1],givenPoints[i][2], givenPoints[j][1], givenPoints[j][2]));
        }
    }
	vector<vector<double>> Mat=vector<vector<double> >(V, vector<double>(V, 0));
	PrimMST(graph, Mat, total_distance,givenPoints);
    cout<<"TotalDistance: "<<total_distance<<endl;
	vector<bool> visited(V, false);
	//dfs(0, visited,Mat);
    dfsOrder(0, visited,Mat);
