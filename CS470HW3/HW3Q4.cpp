...
void KMP(char* pattern, char* txt)
{
  ...
  myfile.open ("trace_example.txt");
    ...
    //(N - i) >= (M - j)
	while ((N - i) >= (M - j)) {
		myfile<<"\n~int j="<<j<<"\tint i="<<i<<endl;
		myfile<<"pattern["<<j<<"]("<<pattern[j]<<")\ttxt["<<i<<"]("<<txt[i]<<")"<<endl;
		...
		if (j == M) {
			myfile<<"-j==M=("<<j<<")"<<endl;
			myfile<<"Found pattern at index "<<i - j<<endl;
			j = lps[j - 1];
		}

		// mismatch after j matches
		else if (i < N && pattern[j] != txt[i]) {
			myfile<<"-i("<<i<<")<N("<<N<<") && pattern[j]("<<pattern[j]<<")!=txt[i]("<<txt[i]<<")"<<endl;
			if (j != 0){
				myfile<<"-j("<<j<<")!=0"<<endl;
				myfile<<"j=lps[j-1]("<<lps[j]<<")"<<endl;
				j = lps[j - 1];}
			else{
				myfile<<"-j("<<j<<")==0"<<endl;
				myfile<<"i=i("<<i<<")+1"<<endl;
...
int main()
{
	...
	// myfile<<"Test Case 1"<<endl;
    // KMP(pattern1, txt1);

    // char txt2[] = "aaabaabacdedfaabaabaaa";
	// char pattern2[]="aabaabaaa";
	// myfile<<"Test Case 2"<<endl;
    // KMP(pattern2, txt2);

    // char txt3[] = "adafccfefbbbfeeccbcfd";
	// char pattern3[]="ecb";
	// myfile<<"Test Case 3"<<endl;
    // KMP(pattern3, txt3);

	char txt3[] = "00100100100010111";
	char pattern3[]="10010001";
